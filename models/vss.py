...
class VSS(nn.Module):
    def __init__(self, in_chans=3, patch_size=4, num_classes=1000, 
            depths=[3, 3, 9, 3], dims=96, out_indices=[0,1,2,3],
...
model_urls = {
    ...
    "vss_base" : "https://github.com/MzeroMiko/VMamba/releases/download/%2320240218/vssmbase_dp05_ckpt_epoch_260.pth"
}
# if True:
...
@register_model
def vss_base(pretrained=False,**kwargs):
    model = VSS(depths=[2, 2, 27, 2], dims=[96, 192, 384, 768], **kwargs)
    if pretrained:
        # url = model_urls['vss_base']
...
if __name__ == "__main__":
    model = vss_base(pretrained=True).to('cuda')
    int = torch.randn(16,3,224,224).cuda()
    out = model(int)
    print(out[0].shape)
    print(out[1].shape)
    print(out[2].shape)
